const addTwoNumbers = require('./index.js');
class ListNode {
    constructor(val) {
        this.val = val;
        this.next = null;
    }
}

test('should pass the test cases', () => {
    let l1 = new ListNode(7);
    l1.next = new ListNode(2);
    l1.next.next = new ListNode(4);
    l1.next.next.next = new ListNode(3);
    let l2 = new ListNode(5);
    l2.next = new ListNode(6);
    l2.next.next = new ListNode(4);
    expect(addTwoNumbers(l1, l2)).toEqual({"next": {"next": {"next": {"next": null, "val": 7}, "val": 0}, "val": 8}, "val": 7});

    let l3 = new ListNode(2);
    l3.next = new ListNode(4);
    l3.next.next = new ListNode(3);  
    let l4 = new ListNode(5);
    l4.next = new ListNode(6);
    l4.next.next = new ListNode(4);    
    expect(addTwoNumbers(l3, l4)).toEqual({"next": {"next": {"next": null, "val": 7}, "val": 0}, "val": 8});

    let l5 = new ListNode(0);
    let l6 = new ListNode(0);
    expect(addTwoNumbers(l5, l6)).toEqual({"next": null, "val": 0});
});